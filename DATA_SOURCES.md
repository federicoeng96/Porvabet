...
**Indagine corner/cartellini, ri-audit successivo: i mercati Exchange
esistono davvero — il codice esatto resta da confermare dal vivo, non più
"forse esistono".** Richiesta esplicita dell'utente (due volte, in sessioni
diverse) di verificare se Betfair Exchange offra davvero mercati corner/
cartellini per il calcio.

**Primo giro** (esito originale, mantenuto per cronologia):
1. **Documentazione ufficiale Betfair**: `docs.developer.betfair.com` e
   ...
   betfair.com**, non solo gli endpoint di login/Betting API.
2. **Libreria `betfairlightweight` installata**: nessun elenco/costante dei
   market type code esiste nel pacchetto — solo un indizio circostanziale
   (`inplayserviceresources.py` ha campi nativi `numberOfCorners`/
   `numberOfYellowCards`/`numberOfRedCards`), non la conferma di un market
   type code per l'exchange.

**Secondo giro, ri-verificato tramite ricerca web** (non solo la sandbox
diretta — anche uno strumento di fetch web assistito da AI, per escludere
che il blocco fosse specifico al metodo usato prima): confermato con fonti
indipendenti reali, non solo circostanziali, che **i mercati corner e
cartellini/bookings esistono davvero sul Betfair Exchange** (il prodotto
peer-to-peer usato da questo provider, non solo il Betfair Sportsbook, un
prodotto diverso con una API diversa):
- Il materiale educativo ufficiale di Betfair sull'Exchange descrive
  esplicitamente esempi di back/lay su "fewer corners"/"more corners" come
  mercato reale dell'exchange.
- Siti indipendenti di trading/recensioni Betfair (non affiliati, es.
  goalprofits.com, predictmarkets.com) confermano mercati corner e
  bookings/cartellini con liquidità reale sull'Exchange per le partite
  principali, e citano persino sviluppo prodotto 2025 ("Asian Corners
  line") — quindi un'area attiva, non dismessa.
- **Ancora non trovato**: il market type code letterale (es. "CORNERS_1X2"
  o simile) e la convenzione di naming dei runner — ogni fonte di
  documentazione ufficiale Betfair resta bloccata (stesso HTTP 403 su tutte
  le sotto-directory, confermato di nuovo anche tramite lo strumento di
  fetch web), e nessuna fonte terza documenta il codice letterale.
  **Indovinarlo resterebbe la "fonte dati inventata" che questo progetto
  evita.**
- **Soluzione implementata invece di aspettare**:
  `BetfairExchangeOddsProvider.discover_market_types_for_match` (nuovo
  metodo) chiama `listMarketTypes` di Betfair — un'operazione reale e
  documentata dell'API-NG — per la fixture specifica, e restituisce
  l'elenco vero di ogni market type che Betfair offre per quella partita,
  segnalando quelli che sembrano corner/cartellini (match euristico su
  "CORNER"/"BOOKING"/"CARD" nel codice restituito, mai usato per scegliere
  un mercato automaticamente). Non chiede di indovinare nulla: chiede a
  Betfair stesso cosa esiste davvero. `scripts/discover_betfair_market_types.py`
  esegue questo per ogni fixture reale già in DB — l'utente lo lancia dal
  proprio computer (dove Betfair non è bloccato) e riporta l'output, dopo
  di che il market type/naming reale può essere collegato a
  `get_odds_for_match` con lo stesso rigore già usato per
  `MATCH_ODDS`/`OVER_UNDER_25`.
- **Conclusione aggiornata**: non più "forse non esistono affatto" — **esistono,
  confermato da fonti reali indipendenti** — ma restano "n/d" nel Decision
  Layer (probabilità del modello mostrata, Value/Alert non calcolabili) fino
  a quella conferma dal vivo del codice esatto, per lo stesso motivo di
  sempre: mai un codice API indovinato.
